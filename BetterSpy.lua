...
local Simple = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.new(1, 1, 1),AutoButtonColor = false,BackgroundTransparency = 1,Position = UDim2.new(0, 5, 0, 0),Size = UDim2.new(0, 57, 0, 18),Font = Enum.Font.SourceSansBold,Text =  "BetterSpy by Ribbined",TextColor3 = Color3.new(1, 1, 1),TextSize = 14,TextXAlignment = Enum.TextXAlignment.Left})
...
--- @param type string The type of the remote being logged (either 'function' or 'event' or 'uevent')
--- @param args any
--- @param remote any
...
function newRemote(type, data)
    ...
    local ColorbarColor = type == "event" and Color3.fromRGB(255, 242, 0) or Color3.fromRGB(99, 86, 245)
    if type == "uevent" then
       ColorbarColor = Color3.fromRGB(255, 165, 0) 
    end
    ...
    local ColorBar = Create("Frame",{Name = "ColorBar",Parent = RemoteTemplate,BackgroundColor3 = ColorbarColor,BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 1),Size = UDim2.new(0, 7, 0, 18),ZIndex = 2})
            ...
            xpcall(function()
                for i, v in next, args do
                    ...
                    if typeof(v) == "buffer" then
                        ...
                        gen = gen .. "buffer --[[Custom buffer handling here]]"
                    elseif type(v) ~= "Instance" and type(v) ~= "userdata" then
                        gen = gen .. "object"
...
ufunctions = {
    ...
    buffer = function(u)
        ...
        local result = buffer.tostring(u)
...
function remoteHandler(data)
    ...
    if (data.remote:IsA("RemoteEvent") or data.remote:IsA("UnreliableRemoteEvent")) and lower(data.method) == "fireserver" then
        if data.remote:IsA("UnreliableRemoteEvent") then
            newRemote("uevent", data)
        else
...
local newindex = function(method,originalfunction,...)
    if typeof(...) == 'Instance' then
        ...
        if remote:IsA("UnreliableRemoteEvent") or remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") then
...
function()
    setclipboard("https://discord.gg/5nWJ8ZMaXk")
    TextLabel.Text = "Credit to Simple Spy V3 for the original project\nI, Ribbined, made a ton of edits such as fixing support for new values such as;\n buffer lib support, UnreliableRemoteEvents, and more!"
end)
